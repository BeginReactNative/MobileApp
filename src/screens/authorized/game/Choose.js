...
export default class swiper extends Component {
  ...
  componentWillMount() {
    return fetch('http://api-dot-hola-edu.appspot.com/api?action=getCards&ids=[4577050144800768]')
    ...
    render() {
        ...
        return (
            ...
            >
                {
                    itemList.map((item, i) => {
                        return(<View key={i} style={{backgroundColor:'red',flex:1}}>
                            <RenderItemView
...
const renderPagination = (index, total, context) => {
    ...
    for (let i = 0; i < total; i++) {
        items.push(
            <TouchableOpacity key={i} onPress={() => {
                context.scrollBy(i);
                console.log(i);
                this.test.scrollTo({x: (total.index * 20), y: 0, animated: false});
            }}>
    ...
    return (
        <View style={{
            ...
            <ScrollView horizontal style={{flex: 1, backgroundColor: 'white'}} ref={(c) => this.test = c}
