export default [{
	id: 5766596232478720,
	vote: 0.0,
	voteNum: 0,
	ranking:0,
	createDate:1495645200000,
	endDate:1498323600000,
	lastUpdate:1495720048540,
	privacy:0,
	status:1,
	cost:0.0,
	shortDescription:'Học viên được trang bị các kĩ năng cơ bản làm bài thi Học viên hình thành các kĩ năng phân tích và tư duy logic hợp lýKĩ năng viết và phát triển bài luận được nâng caosĐiểm SAT kì vọng: 900-1200',
	avatar: 'https://apusvietnam.com/wp-content/uploads/2015/04/apus_com_eng.jpg',
	description: 'Trong lớp SAT Intermediate, học viên được trang bị các kỹ năng đọc, viết, toán ở mức độ nâng cao hơn. Ngoài ra học viên sẽ được giới thiệu và thực hành các chiến thuật giúp nâng cao điểm số. Những kĩ năng đạt được sau khoá học này còn giúp học sinh rất nhiều trong quá trình viết luận để xin học bậc Đại học ở Mỹ. Phương pháp dạy và tài liệu được biên soạn riêng bám sát với format SAT mới',
	lessonIds: [5713990399295488,5151040445874176,6276940352716800,4869565469163520,5995465376006144,5432515422584832,6558415329427456,4728827980808192,5854727887650816,5291777934229504],
	documentIds:[],
	documents:[],
	listTopic:[],
	itemInfo: {"id":5766596232478720,"rateValue":5.0,"rateCount":4,"commentNum":0,"lastUpdate":-1},
	background: '#2980B9',
	imgSrc: 'https://s3.amazonaws.com/uifaces/faces/twitter/rem/128.jpg',
	imgBorderColor: '#015389',
	name: 'Intermediate SAT',
	ownerName: 'Admin',
	mobileNo: '0491 570 156',
	location: 'Sydney, Australia',
	role: 'Starting the company in sales, James is now responsible for overseeing all staff training. James mainly focuses on getting new employees up to speed with the practices and procedures Hunter & Co has continually refined over the last 50 years.',
	memberIds:[1,2,3,4,5]
}, {
	id: 5766596232478721,
	vote: 0.0,
	voteNum: 0,
	ranking:0,
	createDate:1495645200000,
	endDate:1498323600000,
	lastUpdate:1495720048540,
	privacy:0,
	status:1,
	cost:0.0,
	shortDescription:'',
	avatar: '',
	description: '',
	lessonIds: [5713990399295488,5151040445874176,6276940352716800,4869565469163520,5995465376006144,5432515422584832,6558415329427456,4728827980808192,5854727887650816,5291777934229504],
	documentIds:[],
	documents:[],
	listTopic:[],
	itemInfo: {"id":5766596232478720,"rateValue":5.0,"rateCount":4,"commentNum":0,"lastUpdate":-1},
	background: '#27AE60',
	imgSrc: 'https://s3.amazonaws.com/uifaces/faces/twitter/glif/128.jpg',
	imgBorderColor: '#086C32',
	name: 'React Native',
	ownerName: 'Language Link',
	mobileNo: '0491 570 157',
	location: 'Brisbane, Australia',
	role: "Isaac has overseen all of Hunter and Co's creative efforts for the last five years. He's ability to generate a shared vision between teams throughout the company has been his biggest achievement.",
	memberIds:[1,2,5]
}, {
	id: 5766596232478722,
	vote: 0.0,
	voteNum: 0,
	ranking:0,
	createDate:1495645200000,
	endDate:1498323600000,
	lastUpdate:1495720048540,
	privacy:0,
	status:1,
	cost:0.0,
	shortDescription:'',
	avatar: '',
	description: '',
	lessonIds: [5713990399295488,5151040445874176,6276940352716800,4869565469163520,5995465376006144,5432515422584832,6558415329427456,4728827980808192,5854727887650816,5291777934229504],
	documentIds:[],
	documents:[],
	listTopic:[],
	itemInfo: {"id":5766596232478720,"rateValue":5.0,"rateCount":4,"commentNum":0,"lastUpdate":-1},
	background: '#9B27AE',
	imgSrc: 'https://s3.amazonaws.com/uifaces/faces/twitter/kfriedson/128.jpg',
	imgBorderColor: '#6A067A',
	name: 'Inter SAT',
	ownerName: 'Kien Nguyen',
	mobileNo: '0491 570 158',
	location: 'Sydney, Australia',
	role: "In Sarah's short time with the company, she is now a key figure in the sales team for the Sydney and outer region. Her excellent communication skills has opened up the door to let her mentoring any new hires in sales.",
	memberIds:[1,2,3,4,5.6,7,8,9]
}, {
	id: 5766596232478723,
	vote: 0.0,
	voteNum: 0,
	ranking:0,
	createDate:1495645200000,
	endDate:1498323600000,
	lastUpdate:1495720048540,
	privacy:0,
	status:1,
	cost:0.0,
	shortDescription:'',
	avatar: '',
	description: '',
	lessonIds: [5713990399295488,5151040445874176,6276940352716800,4869565469163520,5995465376006144,5432515422584832,6558415329427456,4728827980808192,5854727887650816,5291777934229504],
	documentIds:[],
	documents:[],
	listTopic:[],
	itemInfo: {"id":5766596232478720,"rateValue":5.0,"rateCount":4,"commentNum":0,"lastUpdate":-1},
	background: '#e67e22',
	imgSrc: 'https://s3.amazonaws.com/uifaces/faces/twitter/9lessons/128.jpg',
	imgBorderColor: '#9D4F09',
	name: 'SAT Beginner',
	ownerName: 'Tech Lead',
	mobileNo: '0491 570 110',
	location: 'Melbourne, Australia',
	role: "In recent years Hunter & Co's website and accompaning app has undergone a massive face lift. Srinivas was responsible for the exploration and planning of the new technology used. He now works on maintaining and continually improving the website.",
	memberIds:[1,2,3,4,5]
},
{
	id: 5722383033827328,
	vote: 0.0,
	voteNum: 0,
	ranking:0,
	createDate:1495645200000,
	endDate:1498323600000,
	lastUpdate:1495720048540,
	privacy:0,
	status:1,
	cost:0.0,
	shortDescription:'',
	avatar: '',
	description: '',
	lessonIds: [5713990399295488,5151040445874176,6276940352716800,4869565469163520,5995465376006144,5432515422584832,6558415329427456,4728827980808192,5854727887650816,5291777934229504],
	documentIds:[],
	documents:[],
	listTopic:[],
	itemInfo: {"id":5766596232478720,"rateValue":5.0,"rateCount":4,"commentNum":0,"lastUpdate":-1},
	background: '#e67e22',
	imgSrc: 'https://s3.amazonaws.com/uifaces/faces/twitter/9lessons/128.jpg',
	imgBorderColor: '#9D4F09',
	name: 'SAT Beginner',
	ownerName: 'Tech Lead',
	mobileNo: '0491 570 110',
	location: 'Melbourne, Australia',
	role: "In recent years Hunter & Co's website and accompaning app has undergone a massive face lift. Srinivas was responsible for the exploration and planning of the new technology used. He now works on maintaining and continually improving the website.",
	memberIds:[1,2,3,4,5]
}
];